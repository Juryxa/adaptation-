...
((window, document, Math) => {
  ...
  let container, picker, colorArea, colorAreaDims, colorMarker, colorPreview, colorValue, clearButton, hueSlider,
    hueMarker, alphaSlider, alphaMarker, currentEl, currentFormat, oldColor;
  const settings = {
    el: '[data-coloris]',
    parent: 'body',
    theme: 'default',
  ...
  function HSVAtoRGBA(hsva) {
    ...
    return {r: Math.round(red * 255), g: Math.round(green * 255), b: Math.round(blue * 255), a: hsva.a};
  }
  ...
  function RGBAtoHSVA(rgba) {
    ...
    return {h: hue < 0 ? hue + 360 : hue, s: Math.round(saturation * 100), v: Math.round(value * 100), a: rgba.a};
  }
  ...
  function init() {
    ...
    picker.innerHTML = `<input id="clr-color-value" class="clr-color" type="text" value="" spellcheck="false" aria-label="${settings.a11y.input}">` +
